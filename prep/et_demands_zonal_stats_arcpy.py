...
def main(gis_ws, input_soil_ws, cdl_year, zone_type='huc8', 
    ...
    ##if not arcpy.Exists(et_cells_path):
    ##    ## Join the input shapefile to the zones and read in the matches
    ...
    if zone_field not in field_list:
        logging.debug('  {0}'.format(zone_field))
        arcpy.AddField_management(et_cells_path, zone_field, 'TEXT', '', '', 8)
    ...
    ##arcpy.CalculateField_management(
    ##    et_cells_path, irrig_flag_field, irrig_flag_default, 'PYTHON')

    ##logging.info('  {0:10s}: {1}'.format(permeability_field, permeability_default))
    ##arcpy.CalculateField_management(
    ##    et_cells_path, permeability_field, permeability_default, 'PYTHON')
    ##logging.info('  {0:10s}: {1}'.format(soil_depth_field, soil_depth_default))
    ##arcpy.CalculateField_management(
    ##    et_cells_path, soil_depth_field, soil_depth_default, 'PYTHON')
    ##logging.info('  {0:10s}: {1}'.format(aridity_field, aridity_default))
    ##arcpy.CalculateField_management(
    ##    et_cells_path, aridity_field, aridity_default, 'PYTHON')

    ##logging.info('  {0:10s}: {1}'.format(dairy_cutting_field, dairy_cutting_default))
    ##arcpy.CalculateField_management(
    ##    et_cells_path, dairy_cutting_field, dairy_cutting_default, 'PYTHON')
    ##logging.info('  {0:10s}: {1}'.format(beef_cutting_field, beef_cutting_default))
    ##arcpy.CalculateField_management(
    ##    et_cells_path, beef_cutting_field, beef_cutting_default, 'PYTHON')
...
def arg_parse():
    ...
    parser.add_argument(
        '--zone', default=8, metavar='STR', type=str,
        choices=('huc8', 'huc10', 'county'), help='Zone type')
    parser.add_argument(
