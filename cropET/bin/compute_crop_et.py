...
def compute_crop_et(data, et_cell, crop, foo, foo_day):
    ...
    logging.debug(
        ('compute_crop_et(): depl_surface %.6f  wt_irr %.6f  depl_ze %.6f  depl_zep %.6f') %
        (foo.depl_surface, foo.wt_irr, foo.depl_ze, foo.depl_zep))
    ...
    ## [140820] changed the tests below to round(watin_ze?,6) in both py/vb for consistency
    ...
    #' depl_ze should be initialized at start of each new crop
