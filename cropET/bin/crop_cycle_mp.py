...
def crop_cycle(data, et_cell, debug_flag=False, vb_flag=False, mp_procs=1):
    ...
    ## CGM - I don't think the following comments are true any more
    ...
    ##'''
    ##   ' start with crop type 44 (bare soil, winter mulch) and run through last crop first
    ##   '<------ specific value for crop number
    ...
    for crop_num, crop in sorted(et_cell.crop_params.items()):
        ...
        if et_cell.crop_flags[crop_num] == 0:
            ...
            continue               
        ##else:      
        ##    logging.info('  Crop %2d %s' % (crop_num, crop))                 
        
        ## Load inputs for each crop
        ## Force debug flag False
...
def crop_day_loop_mp(tup):
    ...
    Returns:
        None
    """
    return crop_day_loop_sp(*tup)
            
def crop_day_loop_sp(data, et_cell, crop, debug_flag=False, vb_flag=False):
    ...
    Returns:
        None
    """ 
    print('  Crop %2d %s' % (crop.class_number, crop.name))  
